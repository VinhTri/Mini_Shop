...
export function ProductCommunity({ product }: { product: Product }) {
  ...
  return <>
    ...
    {chatOpen && <div className="product-chat-layer" onMouseDown={(e) => { if (e.target === e.currentTarget) setChatOpen(false) }}><section className="product-chat">
      ...
      {!user ? <div className="chat-login"><b>Đăng nhập để trò chuyện với MeoShop</b><p>Cuộc tư vấn sẽ được lưu riêng cho tài khoản của bạn.</p><Link className="btn" to="/login" state={{ from: `/products/${product.id}` }}>Đăng nhập</Link></div> : <><div className="chat-messages">{!chat?.messages.length && <div className="chat-welcome">Chào bạn, hãy gửi câu hỏi về sản phẩm. Admin sẽ phản hồi sớm.</div>}{chat?.messages.map(item => <div key={item.id} className={`chat-bubble ${item.fromAdmin ? 'admin' : 'user'}`}><small>{item.fromAdmin ? 'MeoShop' : 'Bạn'}</small><p>{item.content}</p></div>)}</div><form onSubmit={(e) => void sendMessage(e)}><textarea value={message} onChange={e => setMessage(e.target.value)} rows={2} maxLength={1500} placeholder="Nhập câu hỏi về sản phẩm..."/><button className="btn" disabled={sending || !message.trim()}>Gửi</button></form></>}
