...
export function ProductReviewsPanel({ productId }: { productId: number }) {
  ...
  return <section className="product-review-admin">
    <header><div><p>Phản hồi khách hàng</p><h2>Đánh giá sản phẩm</h2><span>Xem và chỉnh sửa phản hồi của MeoShop cho riêng sản phẩm này.</span></div><div className="product-review-summary"><strong>{average ? average.toFixed(1) : '—'}</strong><span>★★★★★</span><small>{reviews.length} đánh giá</small></div></header>
    {error && <p className="product-form-error">{error}</p>}
    {!reviews.length ? <div className="product-review-empty"><span>☆</span><b>Chưa có đánh giá</b><p>Khi khách hàng đánh giá sản phẩm, nội dung sẽ xuất hiện tại đây.</p></div> : <div className="product-review-admin-list">{reviews.map(review => <article key={review.id}><header><span className="reviewer-avatar">{review.userName.charAt(0)}</span><div><b>{review.userName}</b><span className="review-stars">{'★'.repeat(review.rating)}{'☆'.repeat(5-review.rating)}</span></div><em className={review.adminReply?'answered':'waiting'}>{review.adminReply?'Đã phản hồi':'Chờ phản hồi'}</em></header><blockquote>{review.comment}</blockquote><form onSubmit={(e)=>void reply(e, review)}><label>Phản hồi của MeoShop<textarea value={drafts[review.id] ?? ''} onChange={e=>setDrafts(current=>({...current,[review.id]:e.target.value}))} maxLength={1500} rows={3} placeholder="Viết phản hồi cho khách hàng..."/></label><div><small>{(drafts[review.id] ?? '').length}/1500</small><button className="btn" disabled={savingId===review.id || !(drafts[review.id] ?? '').trim()}>{savingId===review.id?'Đang lưu...':review.adminReply?'Lưu chỉnh sửa':'Gửi phản hồi'}</button></div></form></article>)}</div>}
  </section>
}
